...
const long topDishLeftData[][3] = {
    {5000, 60, 1},   // 5000ms ON, 60ms OFF, repeat 1 time
    {100, 100, 3},    // 100ms ON, 100ms OFF, repeat 3 times
    {2000, 100, 1},  // 2000ms ON, 100ms OFF, repeat 1 time
    {500, 200, 5}    // 500ms ON, 200ms OFF, repeat 5 times
};

const long topDishRightData[][3] = {
    {4990, 40, 1},   // 4990ms ON, 40ms OFF, repeat 1 time
    {500, 500, 2},  // 500ms ON, 500ms OFF, repeat 2 times
    {1000, 200, 1}, // 1000ms ON, 200ms OFF, repeat 1 time
    {100, 50, 4}    // 100ms ON, 50ms OFF, repeat 4 times
};

// Bottom Dish LEDs
const long bottomDishLeftData[][3] = {
    {5100, 1000, 1},  // 5100ms ON, 1000ms OFF, repeat 1 time
    {250, 250, 2},    // 250ms ON, 250ms OFF, repeat 2 times
    {100, 500, 1},   // 100ms ON, 500ms OFF, repeat 1 time
    {50, 100, 4}     // 50ms ON, 100ms OFF, repeat 4 times
};

const long bottomDishRightData[][3] = {
    {5100, 1000, 1},  // 5100ms ON, 1000ms OFF, repeat 1 time
    {500, 1000, 2}, // 500ms ON, 1000ms OFF, repeat 2 times
    {200, 100, 1},   // 200ms ON, 100ms OFF, repeat 1 time
    {100, 50, 4}   // 100ms ON, 50ms OFF, repeat 4 times
};

// Front LED
const long frontLedData[][3] = {
    {100, 900, 3},   // 100ms ON, 900ms OFF, repeat 3 times
    {500, 100, 1},    // 500ms ON, 100ms OFF, repeat 1 time
    {200, 500, 2},   // 200ms ON, 500ms OFF, repeat 2 time
    {1000, 200, 4} // 1000ms ON, 200ms OFF, repeat 4 times
};

// Blue Motor LEDs
const long blueMotorLeftData[][3] = {
    {1990, 4000, 1},  // 1990ms ON, 4000ms OFF, repeat 1 time
    {50, 50, 2},     // 50ms ON, 50ms OFF, repeat 2 time
    {500, 100, 1},   // 500ms ON, 100ms OFF, repeat 1 time
    {100, 50, 3}    // 100ms ON, 50ms OFF, repeat 3 times
};

const long blueMotorRightData[][3] = {
    {1990, 4000, 1}, // 1990ms ON, 4000ms OFF, repeat 1 time
    {1000, 1000, 2},// 1000ms ON, 1000ms OFF, repeat 2 time
    {250, 500, 1},  // 250ms ON, 500ms OFF, repeat 1 time
    {100, 100, 3}  // 100ms ON, 100ms OFF, repeat 3 times
};

// Red Motor LEDs
const long redMotorLeftData[][3] = {
    {1000, 50, 1},   // 1000ms ON, 50ms OFF, repeat 1 time
    {200, 200, 3},   // 200ms ON, 200ms OFF, repeat 3 times
    {50, 100, 1},     // 50ms ON, 100ms OFF, repeat 1 time
    {100, 50, 4}  // 100ms ON, 50ms OFF, repeat 4 times
};
...
const long redMotorRightData[][3] = {
    ...
    {500, 500, 2}, // 500ms ON, 500ms OFF, repeat 2 times
    {100, 500, 1},  // 100ms ON, 500ms OFF, repeat 1 time
    {50, 50, 3} // 50ms ON, 50ms OFF, repeat 3 times
};

// Deflector LED
const long deflectorLedData[][3] = {
    {50, 50, 2},   // 50ms ON, 50ms OFF, repeat 2 time
    {100, 100, 3}, // 100ms ON, 100ms OFF, repeat 3 time
    {200, 100, 1},   // 200ms ON, 100ms OFF, repeat 1 time
    {50, 250, 4}  // 50ms ON, 250ms OFF, repeat 4 times
};
